...
class high(Dataset):
    def __init__(self, data_root, transform=None, train=False, flip=False):
        ...
        for img_name in self.img_list:
            ...
            gt_path = f"{data_root}/{prefix}/ground-truth/{img_name}"
...
def build(image_set, args):
    ...
    if image_set == 'train':
        train_set = high(data_root, train=True, transform=transform, flip=True)
        return train_set
    elif image_set == 'val':
        val_set = high(data_root, train=False, transform=transform)
        return val_set
